...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (errorParam === 'google_auth_failed') {
      ...
      // Cleanup the URL to prevent error persisting on refresh
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // CLEAR old session data before starting new login to prevent token crossover
      localStorage.clear();
      sessionStorage.clear();

      const data = await api.login({ email, password });
      localStorage.setItem('hiring_ai_token', data.access_token);
      localStorage.setItem('hiring_ai_role', data.role);
      localStorage.setItem('hiring_ai_email', data.email);
      localStorage.setItem('hiring_ai_name', data.name);
      
      // Force a full page reload to ensure all axios interceptors and React state are fresh
      window.location.href = data.role === 'admin' ? '/dashboard' : '/dashboard/my-tasks';
    } catch (err: any) {
      if (err.response?.data?.detail?.error === 'email_not_verified') {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('hiring_ai_email', email);
      // Removed hardcoded admin role - use the role returned from the backend or the token
      if (res.role) localStorage.setItem('hiring_ai_role', res.role);
      
      // Role-based redirection per request logic
      if (res.role === 'admin') {
  ...
  const handleGoogleLogin = () => {
    // [NUCLEAR] Clear all old session state before initiating OAuth to prevent crossover
    localStorage.clear();
    sessionStorage.clear();
        ...
        <div className="w-full lg:w-[50%] flex flex-col relative bg-gradient-to-br from-[#FDFBF2] to-[#F1EBD0] dark:from-slate-800 dark:to-slate-900 p-8 sm:p-12 lg:p-16">
          ...
          <div className="flex-1 flex flex-col items-center justify-center w-full relative z-10 pt-4 lg:pt-0">
            <div className="w-full max-w-[340px]">
              <h1 className="text-[1.75rem] leading-tight text-slate-800 dark:text-white font-medium text-center mb-1">Welcome back</h1>
              <p className="text-slate-500 dark:text-slate-400 text-[13px] text-center mb-10">Sign in to your intelligent hiring workspace</p>
              ...
              {showOtpModal ? (
                <div className="w-full flex flex-col items-center mt-6 animate-fade-in">
                  ...
                  <form onSubmit={handleVerifyOtp} className="w-full flex flex-col items-center">
                    <input autoFocus required maxLength={6} type="text" placeholder="123456" className="w-[200px] text-center text-2xl tracking-[0.5em] font-mono px-4 py-3 rounded-2xl bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-white/10 shadow-sm outline-none focus:ring-2 focus:ring-[#FACC15] transition-all text-slate-700 dark:text-white mb-8" value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, ''))} />
                    
                    <button type="submit" disabled={loading || otp.length !== 6} className="w-full py-[0.85rem] rounded-[2rem] bg-[#FACC15] hover:bg-[#EAB308] disabled:bg-[#fde047] disabled:cursor-not-allowed shadow-[0_4px_14px_0_rgba(250,204,21,0.39)] text-slate-900 text-[13px] font-medium transition-all flex items-center justify-center">
                      {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Verify Code'}
                    </button>
                  </form>

                  <div className="mt-8 text-center">
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Didn't receive the email?</p>
                    <button onClick={handleResendOtp} disabled={countdown > 0 || resendLoading} className="text-[13px] font-semibold text-slate-700 dark:text-slate-300 hover:text-blue-600 disabled:opacity-50 transition-colors flex items-center justify-center mx-auto gap-2">
                      {resendLoading && <Loader2 className="w-3 h-3 animate-spin" />}
                      {countdown > 0 ? `Resend code in ${countdown}s` : 'Resend code'}
                    </button>
                  </div>
                  
                  <button onClick={() => {setShowOtpModal(false); setOtp('');}} className="mt-8 text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 flex items-center gap-1 transition-colors">
                    ← Back to sign in
                  </button>
                </div>
              ) : (
                <form className="space-y-[1.15rem]" onSubmit={handleLogin}>
                  
                  <div>
                    <label className="block text-[11px] text-slate-500 dark:text-slate-400 mb-1.5 ml-4 font-medium">Email address</label>
                    <input required type="email" placeholder="amelia@company.com" value={email} onChange={e => setEmail(e.target.value)} className="w-full px-6 py-[0.85rem] rounded-[2rem] bg-white dark:bg-slate-800/80 border-none shadow-sm text-[13px] placeholder:text-slate-300 dark:placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#FACC15] transition-all text-slate-700 dark:text-white" />
                  </div>

                  <div className="relative">
                    <label className="block text-[11px] text-slate-500 dark:text-slate-400 mb-1.5 ml-4 font-medium">Password</label>
                    <div className="relative">
                      <input required type={showPassword ? "text" : "password"} placeholder="••••••••••••••••" value={password} onChange={e => setPassword(e.target.value)} className="w-full pl-6 pr-12 py-[0.85rem] rounded-[2rem] bg-white dark:bg-slate-800/80 border-none shadow-sm text-[13px] placeholder:text-slate-300 dark:placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#FACC15] transition-all text-slate-700 dark:text-white" />
                  ...
                  <div className="flex gap-4 mt-4">
                    ...
                    <button 
                      ...
                      className="flex-1 flex items-center justify-center gap-2 py-[0.65rem] rounded-[2rem] border border-slate-300/80 dark:border-white/10 bg-transparent text-[11px] font-semibold text-slate-700 dark:text-slate-300 hover:bg-white/50 dark:hover:bg-slate-800/50 transition-colors shadow-sm"
                    >
                      {/* Google Icon */}
          ...
          <div className="flex justify-between items-center w-full text-[10px] sm:text-[11px] text-slate-500 dark:text-slate-400 mt-12 lg:mt-0 lg:absolute lg:bottom-10 lg:left-0 lg:px-12 xl:px-16 w-full">
            <span>Don't have an account? <Link to="/signup" className="font-semibold text-slate-800 dark:text-white hover:underline underline-offset-2">Sign up</Link></span>
            <span className="underline underline-offset-2 hover:text-slate-700 dark:hover:text-slate-200 cursor-pointer transition-colors">Privacy Policy</span>
          </div>
